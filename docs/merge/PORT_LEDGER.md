...
| Legacy radio migration | `wip` | Sites 1–6 source integrated; shared picker authorization/async scans integrated; association, legacy handoff and OTA remain. Runtime/device validation deferred to V1. `acquire()` refuses foreign sessions. See `RADIO_MIGRATION.md`. |
...
Latest verified full working-snapshot image (2026-09-20 UTC, P5 picker async scan checkpoint):

| Quantity | Bytes |
|---|---:|
| Firmware image (`firmware.bin`) | 6,395,600 |
| Smallest configured OTA app slot | 6,553,600 |
| Free space | 158,000 |
| Warning reserve | 262,144 |
| Shortfall against reserve | 104,144 |
