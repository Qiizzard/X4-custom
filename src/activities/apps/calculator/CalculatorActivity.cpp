...
void CalculatorActivity::render(RenderLock&&) {
  ...
  // Rule 19: use the shared touch-aware header the rest of the firmware uses,
  // so the back affordance and title placement match every other screen.
  const Rect header = TouchHeaderBackButton::headerRect(renderer, mappedInput);
  if (mappedInput.hasTouchHardware()) {
    TouchHeaderBackButton::draw(renderer, header, tr(STR_APP_CALCULATOR), false);
  } else {
    GUI.drawHeader(renderer, header, tr(STR_APP_CALCULATOR));
  }
