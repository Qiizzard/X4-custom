...
int TetrisActivity::randomPiece() { return nextRandom() % 7; }
...
void TetrisActivity::initGame() {
  ...
  screenW -= ml + mr;
  int availH = screenH - mt - mb - metrics.topPadding - 25 - metrics.buttonHintsHeight;
  cellSize = std::min(availH / BOARD_H, (screenW - 45) / (BOARD_W + 4));
  ...
  boardOffsetX = ml + 15;
  boardOffsetY = mt + metrics.topPadding + 25;
  (void)boardPixelW;
  (void)boardPixelH;
...
void TetrisActivity::renderPlaying() const {
  ...
  // Compact header
  renderer.drawText(UI_10_FONT_ID, boardOffsetX, boardOffsetY - 25, tr(STR_APP_TETRIS), true, EpdFontFamily::BOLD);
  ...
  renderer.drawText(SMALL_FONT_ID, panelX, panelY, tr(STR_GAME_SCORE_LABEL));
  ...
  renderer.drawText(SMALL_FONT_ID, panelX, panelY, tr(STR_GAME_LEVEL));
  ...
  renderer.drawText(SMALL_FONT_ID, panelX, panelY, tr(STR_GAME_LINES));
  renderer.drawText(UI_12_FONT_ID, panelX, panelY + 14, linesBuf2, true, EpdFontFamily::BOLD);

  // Next piece
  panelY += 55;
  renderer.drawText(SMALL_FONT_ID, panelX, panelY, tr(STR_GAME_NEXT));
  panelY += 16;
...
void TetrisActivity::renderGameOver() const {
  ...
  char scoreBuf[48];
  snprintf(scoreBuf, sizeof(scoreBuf), tr(STR_TETRIS_SCORE_LINES), score, linesCleared);
