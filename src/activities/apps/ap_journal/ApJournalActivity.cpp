...
const char* ApJournalActivity::owner() const {
  return kind == Kind::History ? kHistory : kind == Kind::Wardriving ? kWardrive : kChanges;
}
...
void ApJournalActivity::scan(bool replaceBaseline) {
  ...
  if (kind == Kind::Changes) {
    ...
    nextScan = millis() + (kind == Kind::History ? kIntervals[interval] : 10000);
  }
  requestUpdate();
}
void ApJournalActivity::loop() {
  ...
  const int count = kind == Kind::Changes ? differenceCount : recordCount;
...
bool ApJournalActivity::createLog() {
  ...
  HalFile file;
  for (int i = 0; i < 100; ++i) {
    snprintf(path, sizeof(path), "%s/%s-%02d.csv", dir, kind == Kind::History ? "history" : "wardrive", i);
...
void ApJournalActivity::render(RenderLock&&) {
  ...
  if (busy)
    ...
    if (kind == Kind::Changes) {
      ...
      draw(text);
      snprintf(text, sizeof(text), tr(STR_AP_INTERVAL),
               unsigned((kind == Kind::History ? kIntervals[interval] : 10000) / 1000));
      draw(text);
      if (recordCount) {
